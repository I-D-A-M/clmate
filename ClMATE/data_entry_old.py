# Line 35 is where the fix is required for speed up.
# Line 640 is the start of commit_to_DB [is this currently saving EVERY assessment?!]
...
class InputWindow(QtGui.QWidget):
    def __init__(self, session_details, CHOSEN_CLASS,
        ...
        ''' This is the section that needs changing! Try only loading a single
            assessment and shunting the set up logic to tabSwitch() to spread
            out the loading time? '''
        for a in range(len(self.CHOSEN_ASSESSMENTS)):
            ...
            self.NUM_QUESTIONS = self.DB.execute(query,(a_string,)).fetchone()[0]
            query = "select qTitle from assessments where aName = ?"
            raw_names = self.DB.execute(query, (a_string,)).fetchall()
            self.QNAMES = []
        ...
        # Initialise all assessments in the stackLayout
    ...
    def createBoundaries(self):
        ...
        query = ("select * from gradeBoundaries where aName = ? "
                 "order by perc desc")
        boundaries = self.DB.execute(query, (self.CHOSEN_ASSESSMENT,)).fetchall()
    ...
    def mainArea(self):
        ...
        NAMES = self.DB.execute(query, (self.CHOSEN_CLASS,)).fetchall()
        self.name_list = []
        ...
        yrGroup = self.DB.execute(query, (self.CHOSEN_CLASS,)).fetchone()[0]
        if yrGroup == 'GCSE':
            query = "select FFT from cohort where teaching_set = ?"
            TARGETS = self.DB.execute(query, (self.CHOSEN_CLASS,)).fetchall()
            targetName = 'FFT'
        elif yrGroup == 'AS':
        ...
        for q in range(self.NUM_QUESTIONS):
            ...
            qMark = self.DB.execute(query, (self.CHOSEN_ASSESSMENT, q+1)).fetchone()
        ...
        already_entry = self.DB.execute(query, (aName, teaching_set)).fetchone()
        ...
        if already_entry is None:
            ...
            class_results = self.DB.execute(query,
    ...
    def handlePaste(self):
        ...
        query = "select count(*) from assessments where aName = ?"
        NUM_QUESTIONS = self.DB.execute(query,(CHOSEN_ASSESSMENT,)).fetchone()[0]
    ...
    def openpaper(self):
        ...
        paper_query = self.DB.execute(query, (CHOSEN_ASSESSMENT,)).fetchone()[0]
    ...
    def openMS(self):
        ...
        ms_query = self.DB.execute(query, (CHOSEN_ASSESSMENT,)).fetchone()[0]
    ...
    def commit_to_DB(self):
        ...
        aID = self.DB.execute(query, (aName,)).fetchone()[0]
        aID = int(aID)
        query = "select count(*) from assessments where aName = ?"
        NUM_QUESTIONS = self.DB.execute(query,(aName,)).fetchone()[0]
        teaching_set = self.CHOSEN_CLASS

        for p in range(self.CLASS_SIZE):
            query = "select UPN from cohort where name = ? and teaching_set = ?"
            UPN = self.DB.execute(query,
            ...
            for q in range(NUM_QUESTIONS):
                ...
                query = ("select pMark from results where UPN = ? "
                         "and aName = ? and qNum = ?")
                already_entry = self.DB.execute(query, (UPN, aName, qNum)).fetchone()
                if already_entry is None:
                    query = ("insert into results (aID, UPN, teaching_set, "
                             "aName, qNum, pMark) values (?,?,?,?,?,?)")
                    self.DB.execute(query,
                               (aID, UPN, teaching_set, aName, qNum, pMark))
                else:
                    query = ("update results set pMark = ? where UPN = ? "
                             "and teaching_set = ? and aID = ? and qNum = ?")
                    self.DB.execute(query, (pMark, UPN, teaching_set, aID, qNum))
        self.DB.commit()
        return 1
